...
Resume a session hibernated after inactivity (Unisoft fork — see [32 - Session Hibernation](./32-session-hibernation.md)).

Reloads the engine from the credentials already on disk, so no QR scan is involved. Idempotent: a
session whose engine is already loaded is returned unchanged. Sending to a hibernated session wakes
it on its own, so this endpoint is only needed to pay the resume latency up front.
...
Returns as soon as the engine is launching: poll `GET /api/sessions/:sessionId` or watch the
`session.status` WebSocket event until it reports `ready`.

**Errors:** `401` · `403` · `404` not found · `409` another node currently holds this session's live engine (multi-node deployments)
...
| `session.status`                                  | The session status transitions                                                                                                                                                                                                        | `{ sessionId, status }` where `status` is one of `created` / `initializing` / `qr_ready` / `authenticating` / `ready` / `disconnected` / `action_required` / `failed` / `hibernated`                                                                                                                                                                                                                                                                                                                                                                                                     |
