...
export class SessionsResource {
  ...
  /**
   * Resume a session hibernated after inactivity (Unisoft fork). Reloads the engine from the
   * credentials already on disk, so no QR scan is involved. Returns as soon as the engine is
   * launching (`status: 'initializing'`) — poll `retrieve` or watch the `session.status` event
   * until it reports `ready`. Idempotent, and unnecessary before a send: sending to a hibernated
   * session wakes it on its own.
   */
  wake(id: string): Promise<SessionResponse> {
    return this.client.request<SessionResponse>({ method: 'POST', path: `/api/sessions/${encodeSegment(id)}/wake` });
  }
