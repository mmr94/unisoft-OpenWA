...
public final class SessionsResource {
    ...
    /**
     * Resume a session hibernated after inactivity (Unisoft fork). The engine is reloaded from the
     * credentials already on disk, so no QR scan is involved; the call returns as soon as the engine
     * is launching, so poll {@code get} or watch {@code session.status} until it reports ready.
     */
    public SessionResponse wake(String id) {
        return client.request(HttpMethod.POST, "/api/sessions/" + encodeSegment(id) + "/wake", null, null, SessionResponse.class);
    }
