...
class SessionsResource:
    ...
    def wake(self, session_id: str) -> SessionResponse:
        """Resume a session hibernated after inactivity (Unisoft fork).

        Reloads the engine from the credentials already on disk, so no QR scan
        is involved. Returns as soon as the engine is launching
        (``status: 'initializing'``) -- poll :meth:`retrieve` or watch the
        ``session.status`` event until it reports ``ready``. Idempotent, and
        unnecessary before a send: sending to a hibernated session wakes it.
        """
        return self._http.request("POST", f"/api/sessions/{quote_segment(session_id)}/wake")
