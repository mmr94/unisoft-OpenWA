...
export class SessionEngineControls {
  ...
  /**
   * @param finalStatus Status the row lands on once the engine is retired. DISCONNECTED for an
   *   ordinary stop; SessionHibernationService passes HIBERNATED so an intentional idle unload
   *   reports one transition instead of flashing `disconnected` on its way there.
   */
  async stop(id: string, finalStatus: SessionStatus = SessionStatus.DISCONNECTED): Promise<Session> {
    const session = await this.requireSession(id);
    ...
    if (engine) {
      ...
      if (!tornDown) {
        ...
        await this.host.updateStatus(id, finalStatus);
