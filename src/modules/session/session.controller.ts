...
export class SessionController {
  ...
  @ApiOperation({
    ...
    description:
      'Reloads the engine of a session that was hibernated after inactivity, reusing the ' +
      'credentials already on disk. Returns as soon as the engine is launching, with status ' +
      '`initializing`: poll GET /sessions/{sessionId} or watch the session.status WebSocket event ' +
      'until it reports `ready`. Idempotent — a session whose engine is already loaded is returned ' +
      'unchanged. Sending to a hibernated session wakes it on its own, so this is only needed to ' +
      'pay the resume latency up front.',
  })
  @ApiParam({ name: 'sessionId', description: 'Session ID' })
  @ApiResponse({ status: 200, description: 'Session resuming', type: SessionResponseDto })
  @ApiResponse({ status: 404, description: 'Session not found' })
  @ApiResponse({
    status: 409,
